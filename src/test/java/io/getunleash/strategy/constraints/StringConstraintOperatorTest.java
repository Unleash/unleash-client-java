...
class StringConstraintOperatorTest {
    ...
    public void shouldBeEnabledWhenEmailDoesNotEndWith() {
        ...
        UnleashContext ctx =
                UnleashContext.builder()
                        .environment("dev")
                        .addProperty("email", "example@something.ai")
                        .build();
        assertThat(strategy.isEnabled(parameters, ctx, constraintList)).isTrue();
    }
        ...
        List<Constraint> constraintList =
                Collections.singletonList(
                        new Constraint(
                                "email",
                                Operator.STR_ENDS_WITH,
                                Arrays.asList("@getunleash.ai", "somerandom-email.com")));
        Map<String, String> parameters = new HashMap<>();
    ...
    public void shouldNotBeDisabledWhenEmailDoesNotEndWith() {
        ...
        UnleashContext ctx =
                UnleashContext.builder()
                        .environment("dev")
                        .addProperty("email", "example@something-else")
                        .build();
        assertThat(strategy.isEnabled(parameters, ctx, constraintList)).isFalse();
    }
    ...
    public void shouldBeEnabledWhenEmailContains() {
        ...
        List<Constraint> constraintList =
                Collections.singletonList(
                        new Constraint(
                                "email", Operator.STR_CONTAINS, Collections.singletonList("some")));
        Map<String, String> parameters = new HashMap<>();
        UnleashContext ctx =
                UnleashContext.builder()
                        .environment("dev")
        ...
        List<Constraint> constraintList =
                Collections.singletonList(
                        new Constraint(
                                ...
                                Collections.singletonList("ample"),
                                true));
        Map<String, String> parameters = new HashMap<>();
        UnleashContext ctx =
                UnleashContext.builder()
